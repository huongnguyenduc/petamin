...
class ChatSearchPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final _cubit = ChatSearchCubit(context.read<PetaminRepository>(), context.read<SocketIoCubit>())..search('h');
    String _query = 'h';
    _scrollListener() {
            ...
            body: Container(
              ...
              child: BlocBuilder<ChatSearchCubit, ChatSearchState>(
                  buildWhen: (previous, current) =>
                      previous.searchResults != current.searchResults || previous.status != current.status,
                  builder: (context, state) {
                    ...
                    if (state.status == ChatSearchStatus.success || state.status == ChatSearchStatus.newQuery) {
                      ...
                      Expanded(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () async {
                              final conversationId = await _cubit.createConversations(chat.userId!);
